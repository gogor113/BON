...
// Upgrade dari v36.0 ke v37.0:
// 1. Menambahkan handler deleteCustomerData untuk Instant Delete
// 2. Memperbaiki cleanupLunasData untuk menghapus data dengan sisa <= 0
// 3. Menambahkan fungsi deleteCustomerDataPermanen
// 4. Mendukung sinkronisasi real-time multi device
...
function doPost(e) {
  try {
    ...
    if (action === "testConnection") {
      ...
      return handleDeleteCustomerData(username, customerName);
    }
...
function handleTestConnection() {
  ...
  return ContentService
    .createTextOutput(JSON.stringify({ 
      success: true, 
      message: "Cloud Backup v37.0 ACTIVE - Instant Delete Ready", 
...
function handleMergeBackupV37(data) {
  ...
  const cleanupResult = cleanupLunasDataV37(username);
  
  addSyncLog(username, "mergeBackup", `Merged ${mergedBons.length} bons, ${mergedPayments.length} payments, cleaned ${cleanupResult.count} customers`);
...
function handleRestoreV37(username) {
  ...
  // Filter only active bons (not fully paid - sisa > 0)
...
function handleCleanupLunasOtomatis(username) {
  const result = cleanupLunasDataV37(username);
  return ContentService
    .createTextOutput(JSON.stringify({ success: true, result: result }))
    .setMimeType(ContentService.MimeType.JSON);
}

// ==================== NEW: HANDLE DELETE CUSTOMER DATA (INSTANT DELETE) ====================
function handleDeleteCustomerData(username, customerName) {
  try {
    const result = deleteCustomerDataPermanen(username, customerName);
    addSyncLog(username, "deleteCustomer", `Deleted customer: ${customerName} (${result.deletedBons} bons, ${result.deletedPayments} payments)`);
...
// ==================== NEW: DELETE CUSTOMER DATA PERMANENTLY ====================
function deleteCustomerDataPermanen(username, customerName) {
...
// ==================== UPDATED: CLEANUP LUNAS DATA (SISA <= 0) ====================
function cleanupLunasDataV37(username) {
  ...
  if (customersToDelete.length > 0) {
    addSyncLog(username, "cleanupLunas", `Deleted ${customersToDelete.length} customers (${deletedBons} bons, ${deletedPayments} payments) - LUNAS total`);
  }
